...
// Global variable to track current PDF context
window.currentPdf = localStorage.getItem('lastOpenedPDF') || '';
window.currentPdfType = localStorage.getItem('lastOpenedPDFType') || '';
...
function updateCurrentPdfContext() {
  ...
  const pdfName = pdfUrl ? (pdfUrl.startsWith('blob:') ? 'Local PDF' : new URL(pdfUrl).hostname) : '';
...
$("#addQuoteBtn").on("click", function() {
  ...
  if (newQuote) {
    ...
    console.log("Adding quote:", newQuote, "for PDF:", pdfUrl);
...
$(document).on("click", ".editQuoteBtn", function(event) {
  ...
  console.log("Edit button clicked for quote ID:", id);
  ...
  getQuoteById(id, (quote) => {
    if (quote) {
      console.log("Quote found, populating input field:", quote.quote);
...
$(document).on("click", ".deleteQuoteBtn", function(event) {
  ...
  if (confirm("Are you sure you want to delete this quote?")) {
    ...
    deleteQuote(id, () => {
      console.log("Quote deleted successfully, updating UI immediately");
...
$(document).on("click", "#modalExportQuotesBtn", function(event) {
  ...
  const pdfName = window.currentPdf ? (window.currentPdf.startsWith('blob:') ? 'Local PDF' : new URL(window.currentPdf).hostname) : '';
  ...
  if (window.currentPdf) {
    ...
    getQuotesByPdf(window.currentPdf, (quotes) => {
      console.log("Exporting quotes for PDF:", pdfName, "Number of quotes:", quotes.length);
...
$("#quotesToggleBtn").on("click", function() {
  console.log("Quotes toggle button clicked");
