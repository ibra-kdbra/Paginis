...
export class TextureLibrary {
  ...
  initThumbs() {
    ...
    const disableOrbit = () => {
      ...
      // Keep scrollWheel enabled to allow Zoom logic via Ctrl+Wheel
      // if (self.flipbook && self.flipbook.options) {
      //   self.flipbook.options.scrollWheel = false;
      // }
    };
    const enableOrbit = () => {
      if (self.targetObject && self.targetObject.stage && self.targetObject.stage.orbitControl) {
  ...
  initOutline() {
    ...
    wrapper[0].addEventListener("mousewheel", stopPropagation, { passive: false, capture: true });
    wrapper[0].addEventListener("DOMMouseScroll", stopPropagation, { passive: false, capture: true });
