...
export class PreviewObject {
  constructor(options) {
    ...
    this.sound.setAttribute("src", options.soundFile + "?ver=" + VERSION);
  ...
  checkCenter(event) {
    ...
    if (target.mode == "css") {
      ...
      let targetX;
      offsetX = target.width / 4;
      wrapperWidth = target.width;
      xLeft = -wrapperWidth / 2;
      xRight = wrapperWidth / 2;

      if (basePage == 0 || isBooklet) {
        targetX = isRTL ? xRight : xLeft;
      } else if (basePage == target.pageCount) {
        targetX = isRTL ? xLeft : xRight;
      } else {
        targetX = isSingle ? (isRTL ? (isEven ? xLeft : xRight) : (isEven ? xRight : xLeft)) : 0;
      }

      if (targetX !== this.centerEnd) {
        this.centerTween = new TWEEN.Tween({ x: currentX })
          .delay(0)
          .to({ x: targetX }, target.duration)
          .onUpdate(function () {
