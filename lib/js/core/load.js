...
window.appState.subscribe('isRTL', (newValue) => {
    ...
    if (typeof window.flipbookInstance !== 'undefined' && window.flipbookInstance) {
        window.flipbookInstance.direction = newValue ? 2 : 1;
    }
});
...
function loadFlipbook(pdfUrl, rtlMode, page, pdfId) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            const hasPages = $("#flipbookContainer .page").length > 0;
            const hasError = $("#flipbookContainer").find('[style*="color: red"]').length > 0;
            const defaultPdf = window.appState.get('currentPdf');

            // If PDF failed to load and it's not the default PDF, try fallback
            if (!hasPages && !hasError && pdfUrl !== defaultPdf) {
