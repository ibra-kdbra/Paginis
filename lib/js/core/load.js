...
function loadFlipbook(pdfUrl, rtlMode, page, pdfId) {
    ...
    // Only cleanup if the source is actually changing to a new blob or URL
    if (window.flipbookInstance && window.flipbookInstance.options && window.flipbookInstance.options.source !== pdfUrl) {
        console.log('Resource source changed, cleaning up PDF resources');
        window.memoryManager.cleanupPDF();
    } else {
        // If it's a reload of the same source (direction toggle), just empty the container
        // but avoid full resource revocation of the blob URL if possible
    ...
    try {
        ...
        // Expose instance globally for direction toggle and other controls
        window.flipbookInstance = flipbookInstance;

        // Register the flipbook instance for memory management
        if (flipbookInstance) {
    ...
    // Add fallback logic for when flipbook fails to load
    setTimeout(() => {
        // Skip fallback if we are already in the middle of another load
        if (isCurrentlyLoading && pdfUrl === window.appState.get('currentPdf')) {
             console.log('Skipping fallback check because another load is in progress');
             return;
        }

        // Check if PDF actually loaded after a short delay
        // DFlip usually creates .df-book-stage, .df-book-wrapper, or canvas elements
        ...
        const defaultPdf = 'https://7uzx5yn03h.ufs.sh/f/aLxFAGHMpUDrHJi2MUlQmdfILsR4SYX9vh5tGa0AxUy31OwF';

        console.log('Load verification - hasContent:', hasDFlipContent, 'hasError:', hasError, 'URL:', pdfUrl);

        // If PDF failed to load and it's not the default PDF, try fallback
        // Fallback if there's an error OR no content loaded
        if ((hasError || !hasDFlipContent) && pdfUrl !== defaultPdf) {
                console.log('PDF failed to load properly, trying fallback default PDF...');
                
                // Set loading to false so fallback can proceed
                isCurrentlyLoading = false;

                const errorMessage = pdfUrl.startsWith('blob:') ?
                    "Local PDF failed to load. Loading default PDF..." :
                    "PDF failed to load. Loading default PDF...";
                ...
                // If this was a stored PDF that failed, clear it from localStorage
                // to prevent repeated failures on app restart
                if (pdfUrl === localStorage.getItem('lastOpenedPDF')) {
                    console.log('Clearing failed stored PDF from localStorage');
                    localStorage.removeItem('lastOpenedPDF');
                    localStorage.removeItem('lastOpenedPDFType');
                    // Reset AppState to default
                    window.appState.set({ currentPdf: defaultPdf, currentPdfType: 'url', currentPdfName: 'Default PDF' });
                }

                // Clear current PDF container and retry with default
                $("#flipbookContainer").empty();
                setTimeout(() => {
                    loadFlipbook(defaultPdf, rtlMode, 1, defaultPdf);
                }, 500);
            }
        ...
        // If this was the default PDF failing, show a different message
        if (pdfUrl === window.appState.get('currentPdf')) {
            $("#flipbookContainer").html(`
                <div style="color: red; padding: 20px; text-align: center;">
                    ...
                    Default PDF failed to load.<br>
                    <small>Please try uploading a different PDF or check your internet connection.</small>
                </div>
            `);
            Toastify({
                text: "Default PDF failed to load. Please try a different PDF.",
                duration: 6000,
                gravity: "bottom",
                position: "right",
                backgroundColor: "#ef4444"
            ...
            // Was a user-provided PDF, try fallback to default
            console.log('User PDF failed, trying default PDF as fallback...');
            Toastify({
                text: "PDF failed to load. Trying default PDF...",
                duration: 3000,
                gravity: "bottom",
                position: "right",
                backgroundColor: "#f59e0b"
            ...
            setTimeout(() => {
                loadFlipbook(window.appState.get('currentPdf'), rtlMode, 1, window.appState.get('currentPdf'));
            }, 1000);
        }
        return;
    }
    ...
    // Store instance globally for AppState listener to use
    if (flipbookInstance) {
        window.flipbookInstance = flipbookInstance;
    }
    
    // Ensure direction is correct for the instance
    if (window.flipbookInstance) {
        window.flipbookInstance.direction = rtlMode ? 2 : 1;
        // Trigger a refresh of the UI to reflect direction change
        if (window.flipbookInstance.ui && window.flipbookInstance.ui.update) {
            window.flipbookInstance.ui.update();
        }
        // Force re-layout for RTL/LTR changes
...
$(document).ready(function () {
    ...
    var pdfId;
    var pdfToLoad = window.appState.get('currentPdf'); // Default to the configured PDF URL
    ...
    if (pdfFromUrl) {
        ...
        if (lastOpenedPDFType === 'local') {
            ...
            pdfId = lastOpenedPDF;
            pdfToLoad = window.appState.get('currentPdf'); // Fall back to default PDF
        } else {
        ...
        pdfId = window.appState.get('currentPdf');
        pdfToLoad = window.appState.get('currentPdf');
    }
