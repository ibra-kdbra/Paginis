...
async function loadLatestVersion() {
  try {
    ...
    const latestVersion = extractLatestVersion(markdown);
    ...
    // Fallback to static version if file can't be loaded
    const versionElements = ['currentVersion', 'footerVersion'];
    versionElements.forEach(id => {
      const element = document.getElementById(id);
      if (element) {
        element.textContent = '3.0.0';
      }
    });
    return '3.0.0';
  }
...
// Function to extract the latest version from changelog markdown
...
// Make function available globally for external access
window.loadLatestVersion = loadLatestVersion;

// Load version on page load if this script is included
document.addEventListener('DOMContentLoaded', function() {
  // Load version for any page that includes this script
  loadLatestVersion();

  // Only continue with changelog loading if we're on the changelog page
  if (!document.getElementById('loadingState')) {
    return; // Not on changelog page
  }
