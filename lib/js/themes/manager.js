...
class ThemeManager {
    ...
    createThemeSelector() {
        // The theme selector is now in the HTML, just add the event listener
        ...
        if (themeSelect) {
            ...
            console.log('Theme selector initialized with current theme:', this.currentTheme);
        }
    }
    ...
    loadSavedTheme() {
        // First try to load from app state (fast loading)
        if (window.appState) {
            const stateTheme = window.appState.get('theme');
            if (stateTheme && this.themes.includes(stateTheme)) {
                this.currentTheme = stateTheme;
                this.applyTheme(this.currentTheme);
                console.log('Theme loaded from app state:', stateTheme);
                return;
            }
        }

        // Fall back to database if app state doesn't have it
        if (window.dbInitialized) {
            import('../quotes/db.js').then(({ getSettings }) => {
                getSettings((settings) => {
                    if (settings && settings.theme && this.themes.includes(settings.theme)) {
                        ...
                        // Also save to app state for consistency
                        if (window.appState) {
                            window.appState.setTheme(settings.theme);
                        }
                    }
                });
            }).catch(error => {
